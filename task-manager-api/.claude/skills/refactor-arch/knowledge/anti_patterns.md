...
- **Recomendação**: Extrair todas as instruções SQL e acessos diretos a dados para a camada de **Model** (`AdminModel`, `HealthModel`, `ReportModel`). Para regras de negócio complexas, relatórios ou orquestração de múltiplos domínios, utilizar a camada de **Service** (`ReportService`, `HealthService`). Manter os Controllers estritamente finos (validação HTTP e retorno JSON), invocando os Models em CRUDs simples (`Controller ➔ Model`) ou Services em fluxos orquestrados (`Controller ➔ Service ➔ Model`), evitando overengineering de serviços desnecessários.
